...
# pho_tokenizer = AutoTokenizer.from_pretrained(PHOBERT_MODEL_NAME)
...
# def load_phobert_matrix(phobert, tokenizer):
#     """
#     Get the word2idx dict and embedding matrix.

#     Args:
#         phobert: The PhoBERT model.
#         tokenizer: The PhoBERT tokenizer.
#     Returns:
#         word2idx: The word2idx dict.
#         embedding_matrix: The PhoBERT model matrix.
#     """
#     word2idx = tokenizer.get_vocab()
#     embedding_matrix = np.zeros((len(word2idx), phobert.config.hidden_size))

#     for word, i in word2idx.items():
#         if word == tokenizer.pad_token:
#             continue
#         embedding_matrix[i] = phobert.embeddings.word_embeddings(torch.tensor(tokenizer.convert_tokens_to_ids([word])[0])).detach().cpu().numpy()

#     # Set embedding for [CLS] token
#     cls_token = tokenizer.cls_token
#     cls_token_idx = tokenizer.convert_tokens_to_ids([cls_token])[0]
#     embedding_matrix[cls_token_idx] = phobert.embeddings.word_embeddings(torch.tensor([cls_token_idx])).detach().cpu().numpy()
...
# def load_bert_matrix(bert_model, tokenizer):
#     """
#     Get the word2idx dict and embedding matrix.
...
#     # print(len(word2idx))
#     # print(word2idx)
#     embedding_matrix = np.zeros((len(word2idx), bert_model.config.hidden_size))

#     for word, i in word2idx.items():
#         if word == tokenizer.pad_token:
#             continue
#         embedding_matrix[i] = bert_model.embeddings.word_embeddings(torch.tensor(tokenizer.convert_tokens_to_ids([word])[0])).detach().cpu().numpy()

#     # Set embedding for [CLS] token
#     cls_token = tokenizer.cls_token
#     cls_token_idx = tokenizer.convert_tokens_to_ids([cls_token])[0]
#     embedding_matrix[cls_token_idx] = bert_model.embeddings.word_embeddings(torch.tensor([cls_token_idx])).detach().cpu().numpy()

#     return word2idx, embedding_matrix

def load_fasttext_matrix(fasttext_file):
    """
    Get the word2idx dict and embedding matrix.

    Args:
        fasttext_file: The fasttext file.
    Returns:
        word2idx: The word2idx dict.
        embedding_matrix: The fasttext model matrix.
    Raises:
        IOError: If fasttext model file doesn't exist.
    """
    if not os.path.isfile(fasttext_file):
        raise IOError("[Error] The fasttext file doesn't exist. ")

    wv = FastText.load(fasttext_file).wv

    word2idx = OrderedDict({"_UNK": 0})
    embedding_size = wv.vector_size
    # Add words from the vocabulary to the dictionary
    ...
    # word2idx = OrderedDict({"_UNK": 0})
    # embedding_size = ft_model.get_dimension()
    ...
    #     else:
    #         print(count)
    #         embedding_matrix[value] = ft_model.get_word_vector(key)
    # print(word2idx)
...
def load_word2vec_matrix(word2vec_file):
    ...
    wv = Word2Vec.load(word2vec_file).wv
...
def load_data_from_prediction_domain(input_file):
  Data = dict()
  with open(input_file) as fin:
      ...
      for eachline in fin:
          ...
          predict_labels = record['predict_labels']
          predict_scores = record['predict_scores']

          Data['id'].append(id)
          Data['labels'].append(labels)
          Data['predict_labels'].append(predict_labels)
          Data['predict_scores'].append(predict_scores)

  return Data

def load_data_on_domain(domain_type, data: dict):
  indices = [i for i, x in enumerate(data['predict_labels']) if int(domain_type) in x]
  for key in data:
      data[key] = [data[key][i] for i in indices if i < len(data[key])]
  
  return data

def load_test_data_on_domain(data: dict, test_data: dict):
  Data = dict()
  Data['id'] = []
  Data['content_index'] = []
  Data['content'] = []
  ...
  for i, id_b in enumerate(test_data['id']):
      matching_indices = [j for j, id_a in enumerate(data['id']) if id_a == id_b]
      for key in Data.keys():
          Data[key] += [data[key][j] for j in matching_indices]
  
  return Data

def load_data_and_labels_domain(args, input_file, word2idx: dict):
    """
    ...
    def _token_to_index(x: list):
        ...
        for item in x:
            if item not in word2idx.keys():
               result.append(word2idx.get('_UNK', 0))
            else:
    ...
    Data['onehot_labels'].append(_create_onehot_labels(section, args.total_classes))
...
def load_data_and_labels_2_levels(args, input_file, word2idx: dict):
    """
    Load research data from files, padding sentences and generate one-hot labels.
...
def load_data_and_labels_3_level(args, input_file, word2idx: dict):
    """
    Load research data from files, padding sentences and generate one-hot labels.
